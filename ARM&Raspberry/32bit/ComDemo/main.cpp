...
void ReadAnswerMode(void)
{
	...
	if (CFCom_InventoryG2(0xFF,arrBuffer,&iTagLength,&iTagNumber) == FALSE)
	{
		//cout<<"Failed"<<endl;
		return;
	}
...
int main()
{
    ...
    //cin>>strDev;
    //if(CFCom_OpenDevice((char *)"/dev/ttyAMA0",115200) == FALSE)
    if(CFCom_OpenDevice((char *)"/dev/ttyUSB0",115200) == FALSE)
    //if(CFCom_OpenDevice(strDev,115200) == FALSE)
    {
    ...
    switch(key)
    {
        case 0://readRFPower
                ...
                07: UartBaudRate*/
            bParamAddr = 0x05;
            if (CFCom_ReadDeviceOneParam(0xFF,bParamAddr, &bValue) == FALSE)
            {
                cout<<"Failed"<<endl;
                return 0;
            }
        ...
        case 1: //SetRF
            ...
            cin >>bValue;
            if (CFCom_SetDeviceOneParam(0xFF,bParamAddr, bValue) == FALSE)
            {
                cout<<"Failed"<<endl;
                return 0;
            }
            ...
            break;
        case 2://WriteTag
            if (CFCom_WriteCardG2(0xFF,Password,Mem,WordPtr,WriteLen,Writedata) == FALSE)
            {
                cout<<"Failed"<<endl;
                return 0;
            }
        ...
        case 6://callback mode
            //CFCom_StartRead(0xFF);
            cout<<"Q: exit"<<endl;
            CFCom_SetCallback(CallBackFunc);
            break;
        case 7://stop
            if (CFCom_StopRead(0xFF) == FALSE)
            {
                cout<<"Failed"<<endl;
                return 0;
            }
